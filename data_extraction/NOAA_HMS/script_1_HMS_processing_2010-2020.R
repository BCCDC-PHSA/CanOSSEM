#####################################################################
# Purpose: Extract HMS data 2010-2020
# Author: namanpaul
...
process_HMS <- function(HMS_input_path, year_val){
  ...
  for (i in seq_along(date_range)) {
    ...
    if(nrow(read_DBF) > 0 & colSums(is.na(read_DBF))[2] == 0){
      ...
      hms_data_input <- st_read(dsn = paste0(HMS_input_path, year_val,'/',date_range[i],"/"),
