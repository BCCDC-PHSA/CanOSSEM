...
MYD_input_path <- "/home/naman/scratch/gdal_complete_test/output/MYD/"
MYD_output_path <- "/home/naman/projects/def-sarahhen/naman/EXTRACTED_data/AOD/MYD/2010/"
...
date_range <- seq(ymd(paste(year_val,"-02-21", sep = "")),
...
MYD_files <- list.files(path = paste0(MYD_input_path, year_val,'/',list_julian_days[i]),
                        pattern = paste0('MYD04_3K.A',year_val,list_julian_days[i]),
                        full.names = T)


# now in this list of MYD tif files, find the ones with the same base name
...
identifiers <- sapply(basename(MYD_files), extract_identifier)
...
for (id in unique_ids) {
  ...
  matching_files <- MYD_files[identifiers == id]
  ...
  save(raster_df_clean,
          file = paste0(MYD_output_path,'MYD_data_',year_val,'_',list_julian_days[i],'_',counter,'.RData'))
   }
}
}
