 #####################################################################
 # Purpose: Primary Viz (retrained CanOSSEM) 
 # Author: namanpaul
...
load('path/model_output/Primary_ranger/MAIN_MODEL/Primary_ranger_prediction_set_with_predictions.RData')
...
Primary_ranger_prediction_set_with_predictions <- Primary_ranger_prediction_set_with_predictions %>%
  mutate(date_val = ymd(str_sub(rCN_date_identifier, 1,10)),
         CanOSSEM_rCN = as.numeric(str_sub(rCN_date_identifier, -6)))



a <- Primary_ranger_prediction_set_with_predictions %>% 
...
ggsave("Primary_ranger_performance_prediction_set.png", a, width = 24, height = 18, units = "cm", dpi = 300)

# importance --------------------------------------------------------------

load('path/model_output/Primary_ranger/MAIN_MODEL/Primary_ranger_importance.RData')
...
importance <- importance %>% 
  ...
  rename(Variables = description,
         Importance = `importance(Primary_ranger)`) %>% 
...
ggsave("Primary_ranger_importance.png", b, width = 24, height = 18, units = "cm", dpi = 300)
...
PM25_r  <- cor(Primary_ranger_prediction_set_with_predictions$predicted_pm25, 
               Primary_ranger_prediction_set_with_predictions$daily_mean_pm25)


lm_fit <- lm(Primary_ranger_prediction_set_with_predictions$predicted_pm25 ~ Primary_ranger_prediction_set_with_predictions$daily_mean_pm25)
...
c <- ggscatter(Primary_ranger_prediction_set_with_predictions, 
...
ggsave("Primary_ranger_P10_r2.png", c, width = 20, height = 12, units = "cm", dpi=300)
...
y <- Primary_ranger_prediction_set_with_predictions$daily_mean_pm25
y_hat <- Primary_ranger_prediction_set_with_predictions$predicted_pm25
(y_bar <- mean(y))
(RSS <- sum((y-y_hat)^2))
(TSS <-  sum((y-y_bar)^2))
(R2 <-  1 - RSS/TSS)
